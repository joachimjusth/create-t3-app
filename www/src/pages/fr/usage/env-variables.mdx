...
Toutes les variables que vous ajoutez à `.env` seront importées en tant que chaînes de caractères, même si leur valeur est censée représenter un type différent. Si vous souhaitez utiliser vos variables d’environnement comme un type différent à l’exécution, vous pouvez utiliser `coerce` de Zod pour convertir la chaîne vers le type souhaité. Une exception sera levée si la conversion échoue.
